...
benchmark, for direct before/after comparison against FT1/FT2.
"""
import csv
import sys
...
CHECKPOINT_PATH = sys.argv[1] if len(sys.argv) > 1 else str(Path.home() / "finetuned-legal-full" / "model.safetensors")
OUTPUT_CSV = sys.argv[2] if len(sys.argv) > 2 else str(Path.home() / "legal-full-results.csv")
...
sd = st.load_file(CHECKPOINT_PATH, device=DEVICE)
